...
interface StreamingTextDOMProps {
  content: string;
  isStreaming: boolean;
  className?: string;
  highlights?: Array<{
...
export interface StreamingTextDOMRef {
  appendToken: (token: string) => void;
  getContent: () => string;
}

export const StreamingTextDOM = forwardRef<
  StreamingTextDOMRef,
  StreamingTextDOMProps
>(({ content, isStreaming, className, highlights = [], messageId }, ref) => {
  ...
  // Direct DOM manipulation for streaming to avoid re-renders
  const appendTokenToDOM = useCallback(
    (token: string) => {
      if (markdownContainerRef.current) {
        // If user is selecting, preserve selection
        if (isSelectionActive && lastSelectionRef.current) {
          const savedSelection = saveSelection();

          // Append token directly to DOM
          const textNode = document.createTextNode(token);
          markdownContainerRef.current.appendChild(textNode);
          ...
          // Normal append without selection preservation
          const textNode = document.createTextNode(token);
          markdownContainerRef.current.appendChild(textNode);
        }
      }
    },
    [isSelectionActive, saveSelection, restoreSelection]
  );
    ...
    <div className={className} ref={containerRef}>
      <div key={renderKey}>
        <MarkdownRenderer
          ...
          messageId={messageId}
        />
        {/* Hidden container for direct DOM manipulation during streaming */}
        {isStreaming && (
          <div
...
StreamingTextDOM.displayName = "StreamingTextDOM";
